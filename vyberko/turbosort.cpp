...
void solve(){
    ...
    FOR(i, (n + 1) / 2)
    {
        cout << oset.order_of_key(aa[i]) << '\n';
        auto it = oset.find_by_order(oset.order_of_key(aa[i]));
        oset.erase(it);
        if (aa[i] != aa[n - i - 1])
        {
            cout << oset.size() - oset.order_of_key(aa[n - i - 1]) - 1 << '\n';
            auto it = oset.find_by_order(oset.order_of_key(aa[n - i - 1]));
            oset.erase(it);
        }
    }
